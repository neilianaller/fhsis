                ...
                success: function(response) {
                    if (response.status === 'success') {
                        ...
                        entries.forEach(entry => {
                            ...
                            // 1️⃣ Try to find input with sex + agegroup + subsection
                            if (entry.agegroup) {
                                input = form.find(
                            ...
                            // 2️⃣ Fallback: input with only sex + subsection
                            if ((!input || !input.length) && entry.sex) {
                                input = form.find(
                                    `input[data-sex="${entry.sex}"][data-subsection="${entry.subsection}"]:not([data-agegroup])`
                                );
                            }

                            // 3️⃣ Set value if input exists
